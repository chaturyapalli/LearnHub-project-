...
const postCourseController = async (req, res) => {
  try {
    ...
    // ✅ Fix: properly access uploaded file(s)
    const uploadedFiles = req.files?.S_content || [];

    if (uploadedFiles.length === 0) {
      return res
        .status(400)
        .send({ success: false, message: "No content files uploaded" });
    }

    const S_content = uploadedFiles.map((file) => file.filename);

    const sections = [];

    if (S_content.length > 1 && Array.isArray(S_title)) {
